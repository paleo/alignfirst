...
2. **Operating-instructions playbook** — [`skills/alignfirst-agent/openclaw-playbook/`](../skills/alignfirst-agent/openclaw-playbook/). `dispatcher.md` routes by surface (thread → `working-session.md`; channel/DM → `channel-handling.md`) and carries the global rules (language, "tickets are labels", projects, `chat_id`). `project-workspace-setup.md` handles worktree/branch setup. None of these is auto-loaded — they're read on demand (see context engineering below).
...
user message
  ...
  → openclaw-playbook/dispatcher.md (read)    layer 2
  → working-session.md | channel-handling.md  layer 2
  → project-workspace-setup.md (if WORK)      layer 2
  → delegate coding to the alignfirst-agent CLI
```

Layer 1 is the only thing OpenClaw injects automatically; everything in layer 2 is pulled in by an explicit file read because nested workspace files and skill files are not auto-loaded.
