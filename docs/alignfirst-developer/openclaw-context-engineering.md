...
The general silence convention is `NO_REPLY`, with the heartbeat exception below. OpenClaw sends `agents.defaults.heartbeat.prompt` verbatim as the scheduled user message. Its stock value ends in an explicit `NO_REPLY`, which conflicts with the settlement token that avoids isolated finalization. The harness and deployment seed therefore replace it with an `[OpenClaw heartbeat poll]` instruction that requires immediate `HEARTBEAT_OK` without reads or tool calls. The former `agents.defaults.heartbeat.includeSystemPromptSection` key is rejected.

OpenClaw 2026.9.3 loses a post-tool `NO_REPLY` from its reply accumulator, then mistakes the silent turn for a missing summary. Its isolated finalizer receives no conversation context and can emit an unsolicited answer. Silent heartbeat turns use the supported `HEARTBEAT_OK` acknowledgement. OpenClaw suppresses this token without triggering finalization. Every user message beginning `[OpenClaw heartbeat poll]`, including a repeat, reads nothing and immediately answers `HEARTBEAT_OK`. A direct `alcode run finished …` message is instead the plugin reply run responsible for the completion report. Ordinary channel and human-turn silence stays on `NO_REPLY`. Disabling block streaming does not avoid the defect.
