...
A direct `alcode run finished …` message arrives through the plugin's reply pipeline and owns the completion report. The enclosing background exec may subsequently emit a native completion notice. When testing that notice, observe the actual event; a generic injected `system event` exercises a different prompt branch.
