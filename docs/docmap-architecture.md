...
| `src/locales/` | Per-language stopwords and plural rules behind the `SearchLocale` contract (`en`, `fr`); an ordered registry in `index.ts`. |
...
4. **`--search`** → `searchDocs()` in `src/search.ts` walks the tree (`collectAllFiles`), reads each file once, and scores it against the query. Query terms are tokenized (split on non-alphanumeric characters), folded — lowercase, unaccented (NFD, combining marks stripped), plural-folded by the first `src/locales/` entry whose rule changes the word — and dropped when any locale flags them as stopwords (an all-stopwords query keeps its terms). Matching is OR: a file matches when any term occurs. Each file exposes three folded haystack tiers — relative path + title (weight 3), summary + read_when (weight 2), body (weight 1) — and a term scores `weight × occurrences` per tier (occurrences = tokens containing the term, capped at 3), plus 1 when some token equals the term exactly. Results order by distinct terms matched, then total score, then path. The top 20 render as file bullets, followed by `… and N more matches` when truncated, or a single `No documents match: <terms>` line when nothing matches. A result whose body matched also carries a grep-style snippet sub-line — `> <line>: <text>`, the body line with the most distinct query terms (earliest on tie), clipped to a 150-character window around the first matched term. Line numbers count raw file lines (frontmatter included), so they are correct when opening the file; metadata-only matches get no snippet, since the bullet already shows the matching fields. Snippets are computed only for the 20 displayed results.
