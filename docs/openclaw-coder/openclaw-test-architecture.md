...
The host CLI owns the matrix: the full `model × scenario × channel × iteration` expansion becomes a cell queue, dispatched to a pool of K worker stacks (`--parallel K`, default `OPENCLAW_TEST_PARALLEL`, fallback 1 — a serial run is a pool of 1). Worker `i` owns Compose project `<base>-w<i>` (`<base>` is the sanitized basename of the project dir, passed via `-p`). Named volumes are namespaced per project and no service publishes host ports, so stacks coexist freely. All three services carry an explicit shared `image:` (`OPENCLAW_TEST_CONSUMER_IMAGE`, injected as `<base>-openclaw-test:latest`), so K worker projects run one build instead of each deriving its own image name; the CLI builds it once up front when missing, since concurrent worker `up`s would otherwise race K builds onto the tag.
