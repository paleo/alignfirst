...
The host CLI owns the matrix loop, nested `model → scenario → channel → iteration`: each cell is one `docker compose run --rm runner --scenario … --channel … --model-id … --model-ref … --iteration-index …` invocation. Between cells the host issues `docker compose up -d --force-recreate --wait bus gateway`, recreating both containers in a single Compose call (`--wait` blocks on healthchecks). Recreation drops in-process gateway state, the bus's in-memory event log, transient container `/tmp` files, and on-process caches that survive a SIGTERM-only restart. The first cell skips recreation when `run` itself just brought the stack up; subsequent cells always recreate. `--reuse-stack` opts out entirely.

Model is the **outermost** dimension: each model's whole sweep runs before the next, so the gateway loads a model's config once and boots on it for that model's entire run — minimal recreates under `--reuse-stack` (one per model boundary). `--model <id|id,id,…|all>` resolves each bare id to a full `provider/model` ref via `OPENCLAW_TEST_MODELS` (`all` alphabetical, an explicit list in CLI order, so `qwen…,claude…` runs qwen first); the CLI renders that ref into `agents.list[id=main].model` of a run-scoped config (`renderRuntimeConfig`, which also expands `${VAR}` secret refs, memoized one temp file per model) that the gateway boots on — the canonical `openclaw.json` is never mutated. At each model boundary the host repoints `OPENCLAW_CONFIG_PATH` at that model's rendered file and **forces** a `--force-recreate` (even under `--reuse-stack`/skip-first-restart) so the gateway reloads the config; shell env wins over Compose `--env-file`. The artifact dir name orders segments `model → scenario → channel` independently of this run order.
...
A conversation spans **multiple sessions** — Discord's channel session plus a per-thread session (the thread session is where the real work happens), and any subagent sessions — each its own `<sessionId>.jsonl` with its own cumulative `data.messagesSnapshot`. Each session flushes its `model.completed` independently, ~2-10s after that session's last turn. So both cost and tool-call parsing aggregate across the **latest `model.completed` per `sessionKey`** (matched by `sessionKey` containing the conversation id, `ts >= runStart`) — taking only the single globally-last event would drop the other sessions. Before parsing, the runner waits for the trajectory to go **quiescent** (`waitForTrajectoryUsage`): no new `model.completed` for the conversation across a settle window, bounded by a max wait — this lets a late-flushing thread session land before the report is written (and before the host moves the files to the artifact dir). Returning on the first event, as it used to, captured only the early channel session.

Cost: sum `usage.cost.total` over the assistant messages in each session's snapshot (event-level `data.usage` is token counts only — no cost), plus the judge's inline `usage` priced via an in-runner table. The trajectory log is written for every provider (OpenClaw computes the cost from the configured per-million-token pricing), so cost and tool-call parsing work under Anthropic, OpenRouter/Qwen, or any other provider. Failing runs that time out before the agent completes report `$0.0000` — no `model.completed` event for the unfinished turn.

Tool calls are read from the same per-session snapshots: walk `data.messagesSnapshot` (OpenClaw's neutral message shape — assistant `toolCall` blocks, `toolResult` messages) and union across sessions, deduped by `toolUseId`. Trajectory events are byte-capped (~256 KB/event); a single session's large snapshot can be truncated to an unusable shape, in which case that session contributes nothing while the others still do (`data.usage` is tiny and unaffected, so cost survives). Scenarios assert on these calls via `ctx.waitForAgentToolCall(predicate, { label })`, which polls the same aggregated view until a match appears (riding out the flush latency) and hard-fails on timeout.
