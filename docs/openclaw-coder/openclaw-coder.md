...
3. **Regression-test harness** — [`openclaw-coder/playbook-test/`](../../openclaw-coder/playbook-test/). A standalone Dockerised consumer of the published `@paleo/openclaw-*` packages that drives the workspace through synthetic Discord/Slack channels and judges the outcome. It bind-mounts the workspace dir, the playbook skill, and the built `@paleo/alcode` package into the gateway. Alcode delegates to the `ALIGNFIRST_CODE_AGENT` selection; the harness intercepts both Claude and Codex subprocesses.
