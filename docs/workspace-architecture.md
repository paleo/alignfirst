...
`workspace remove` against an already-deleted worktree dir takes the same direct-kill path: it can't shell out to `node <devServerScript> down` (the worktree and its `dev-server.mjs` are gone), so it kills the recorded spawn PIDs and drops both registry entries.
