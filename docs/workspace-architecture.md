...
- **Automatic (safe bookkeeping).** `workspace list` drops the `slots.json` (and matching `dev-servers.json`) entry for any orphan with **no live dev-server PIDs** — same harmless pruning as the existing dead-PID sweep. Orphans whose dev-server is still running are left in place and surfaced with a `workspace prune` hint; `list` stays free of process-killing side effects.
- **Explicit (active heal).** `workspace prune` stops every orphan's live spawn PIDs (`stopProcessGroup`), drops both registry entries, then runs `git worktree prune` to clear git's stale admin files.

`workspace remove` against an already-deleted worktree dir takes the same direct-kill path (shared `stopOrphanedDevServer` helper): it can't shell out to `node <devServerScript> down` (the worktree and its `dev-server.mjs` are gone), so it kills the recorded spawn PIDs, drops both registry entries, and runs `git worktree prune` — same full cleanup as `workspace prune`.

**The callback-infra limitation.** For an orphan, the kernel can only kill spawn PIDs — it has no access to the deleted worktree's dev-server config, so it cannot run callback `stop()` (e.g. `docker compose down`). And callback servers leave **no trace** in `dev-servers.json` (PID-keyed on spawn servers only), so the kernel can't even tell whether an orphan had infrastructure. `workspace prune` therefore prints a single generic caveat whenever it stops any process, rather than a precise per-orphan warning. This is the same leak already documented above for manual kills, not a new gap.
