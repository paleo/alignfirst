...
From `alignfirst-dev-kit-tests/`:

```sh
npm run env:up
docker compose -p alignfirst-dev-kit-tests-w1 exec gateway bash
```

The worker-one project is `<consumer-directory>-w1`; here it is `alignfirst-dev-kit-tests-w1`. The stack publishes no host ports. Run bus requests inside its network at `http://bus:43123`.
...
The `transcript_events` columns are `session_id`, `seq`, `event_json`, and `created_at` in OpenClaw 2026.9.5. Confirm them after an upgrade with `.schema transcript_events`.
