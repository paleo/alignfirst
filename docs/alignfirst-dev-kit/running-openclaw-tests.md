---
title: Running the OpenClaw Tests
...
The [`alignfirst-dev-kit-tests`](../../alignfirst-dev-kit-tests/) directory contains the local, manually run regression harness for the `myassistant` reference workspace.

The harness is a standalone consumer of the `@alignfirst/openclaw-*` packages. It has its own `package-lock.json` and is not part of the root npm workspaces. See the upstream documentation for generic mechanics:

- [`packages/openclaw-test/README.md`](../../packages/openclaw-test/README.md) — generic installation, configuration, scenario primitives, and artifact layout.
- [OpenClaw Test Harness Architecture](./openclaw-test-architecture.md) — internals.

This guide documents what is specific to the Dev Kit harness. Run its commands from `alignfirst-dev-kit-tests/` unless stated otherwise.
...
npm run env:build
npm run env:up
npm run e2e -- --model gpt-5.6-terra --channel slack-mock A03-question
npm run env:down
```
...
- `OPENCLAW_CODEX_HOME` — absolute path to a file-backed Codex home. Required for `openai/gpt-5.6-terra`. The gateway mounts it read-only and imports its ChatGPT/Codex credential into the disposable `main` agent's OpenClaw auth store at startup; no OpenAI Platform API key is required. Create a dedicated login so test authentication is isolated from the main Codex session:
  ...
  OpenClaw 2026.9.5 does not use the mounted Codex `auth.json` directly for the OpenClaw runtime. Compose copies the credential to a temporary writable directory, imports it into the disposable `main` agent's auth store, and restarts the gateway because the first process caches authentication state. The source mount remains read-only, and the temporary copy is deleted. `openclaw.json` also routes this subscription credential through the ChatGPT Codex endpoint; using the default OpenAI Platform endpoint produces a 401 response.

  The image build runs `openclaw update repair` followed by `openclaw doctor --fix` to settle deferred plugin state in this OpenClaw release. Recheck both workarounds when changing the pinned OpenClaw version.
...
It retains test-owned gateway logs, scripted-provider requests, configuration, and SQLite restart state under `/tmp/thread-handoff-*`. Omit the environment variable to clean fixtures automatically.
