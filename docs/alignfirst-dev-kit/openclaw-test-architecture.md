...
summary: How the generic `@alignfirst/openclaw-*` harness packages and consumer plugins fit together — bus, gateway, runner, channel plugins, mocked CLIs, and artifact layout.
read_when:
...
| Package | Role |
| --- | --- |
| `@alignfirst/openclaw-test` | Bus, scenario driver, judge, Compose stack, two-Dockerfile pair, CLI (`init` / `env` / `run`). |
...
3. `npm ci --include=dev` — pulls the four `@alignfirst/openclaw-*` packages from the registry.
