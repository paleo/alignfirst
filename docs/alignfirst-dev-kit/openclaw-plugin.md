---
title: AlignFirst Service OpenClaw Plugin
...
`@alignfirst/service-openclaw-plugin` (plugin ID `alignfirst-service`) is the OpenClaw gateway plugin of the AlignFirst Dev Kit. This document states what it is for and the principles that bound its design. The [package README](../../packages/service-openclaw-plugin/README.md) holds the contract and the operations; [`alignfirst-dev-kit.md`](./alignfirst-dev-kit.md) maps the product.
