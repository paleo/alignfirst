...
4. Read the `alignfirst` skill and create `.plans/`. Use the external ticket ID when the request has one. Otherwise atomically reserve the next `nt-N` directory through the skill's no-ticket convention. Write `.plans/{TICKET_ID}/A1-request.md` with the complete creation request. The bot chooses the identifier and writes the request; alcode does neither. A later plans-share setup migrates this content when it replaces the directory with a symlink.
...
The direct main-worktree bootstrap is the creation exception. It ends with the initial commit and the push when a remote destination is known. If the user then requests more changes without a ticket, return to the working-session flow: reserve `nt-N`, create a linked workspace, and delegate from it.
