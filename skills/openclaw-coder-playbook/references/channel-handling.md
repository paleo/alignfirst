...
- **PROJECT known, no TICKET_ID** — Branch on the request:
  - User posed an investigation/advice question (`why X?`, `should we Y?`, `comment X ?`) → delegate the question to alcode without a protocol header (run `alcode --openclaw-guide` first — the delegation manual), **run from the project's directory** (`~/projects/<project>`) so it investigates the right repo. Post alcode's reply back in the thread as a summary — on Discord via a `message` `thread-reply` carrying the `threadId`, never as free-form text.
