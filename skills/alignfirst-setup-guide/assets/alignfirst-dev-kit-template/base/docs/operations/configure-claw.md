...
`models.providers.<provider>.agentRuntime.id` set to `openclaw`; the claw requires OpenClaw's
`exec` and `process` tools.

`openclaw.json` is immutable once [06](../installations/06-security-hardening.md) has run. The root-owned maintenance wrapper contains the claw, refreshes the seed snapshot, unlocks only the requested paths, runs the seed, and restores the hardening policy through an `EXIT` trap.
...
```sh
sudo /usr/local/sbin/alignfirst-claw-maintenance config instructions -- \
  /home/{{SERVICE_USER}}/seed/seed.sh
```
...
Run these commands only after the wrapper reports that hardening was restored and exits 0. A failure leaves the claw contained for diagnosis.
...
- `~/projects/.alignfirst-projects.json` — [update-claw.md](update-claw.md).
