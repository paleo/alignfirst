...
4. Calls optional `config.purgeInfrastructure(ctx)` — destructive teardown (typically `docker compose down -v` to wipe volumes). Runs after the dev-server stop. Also invoked by `workspace prune` and when removing an already-deleted (orphaned) worktree, where `ctx.worktree` no longer exists and teardown must run by name from `ctx.extra`.
...
| `workspace prune` | Heal orphaned workspaces (worktree deleted out-of-band): stop their dev-servers' processes, run `purgeInfrastructure` (by name, from each slot's `extra`), drop their slot + dev-server registry entries, then run `git worktree prune`. See note below |
...
**Orphaned-workspace healing.** When a worktree directory is deleted out-of-band (a manual `rm -rf`, a bare `git worktree remove`), its registry state goes stale. `workspace list` silently drops the entries of orphans with no live dev-server (pure bookkeeping — no process or infrastructure side effects); `workspace prune` heals the rest — it kills each orphan's recorded spawn PIDs, runs `purgeInfrastructure` (with the slot's `worktree`/`slot`/`extra` in context), drops both registry entries, and runs `git worktree prune`. The kernel can't run a deleted worktree's callback `stop()` (its dev-server config is gone), so it kills spawn processes directly; infrastructure teardown relies on `purgeInfrastructure` being cwd-independent. Define `purgeInfrastructure` for this to work; otherwise `prune` prints a caveat about possible leftover callback infrastructure. The `list`-time auto-prune does no teardown — make `finalizeWorktree` idempotent against leftover infra (e.g. force-remove a stale slot-named container before `docker compose up`) to cover that path.
...
- `finalizeWorktree(ctx)` — required callback. Runs in a detached background process after the foreground command returns. Owns infrastructure startup (e.g. `docker compose up -d`), database readiness wait, `npm install` / build, migrations, and seeding. **MUST be idempotent** — `workspace setup` is the documented retry path and re-runs this same callback; idempotency also covers leftover infra from an orphan-and-reuse of the slot (force-remove a stale slot-named container before `up`). **Run `npm install` first** so any later failure leaves a worktree with usable `node_modules/`; otherwise the `workspace setup` retry can't import `@paleo/workspace`. May **return `{ extra }`** — an opaque blob persisted on the slot entry and handed back to `purgeInfrastructure`. Use it only for teardown identifiers you can't re-derive at purge time (e.g. an external resource id); deterministic names (containers/volumes from slot + paths) don't belong here, since `PurgeContext` already carries `slot`, `worktree`, and `mainWorktree`. Failures are logged to `<runtimeDir>/logs/workspace-setup.log` with a `FAILED:` banner.
- `purgeInfrastructure(ctx)` — optional. Called after the dev-server stop on `workspace remove`, and on `workspace prune` / removing an orphaned worktree. The standard pattern is `docker compose down -v` if you use Docker — destructive teardown that wipes volumes, complementing the soft `docker compose down` in the callback `stop()`. **MUST be idempotent and cwd-independent**: `ctx.worktree` may no longer exist (orphan), so branch on its presence and tear down *by name* in that case — derive deterministic names from `ctx.slot` / `ctx.worktree` / `ctx.mainWorktree`, and read `ctx.extra` for any non-derivable identifiers. Swallow errors — infra may already be gone.
