...
export const STARTER_HANDS_OFF_RUBRIC =
  "A thread-opening handoff message from a chat bot. Judge only the message; the user's original request is not shown. Accept: the project, canonical path, ticket, task and detailed request the user supplied, carried faithfully; a missing ticket, project, path or scope that is omitted or marked 'to be defined' in the user's language, since users often give none; a question or request for one of those missing values, including a value the user announced for later ('Envoie le ticket dès qu'il arrive.'); a statement that the work starts now, continues in this thread, or announces what the thread will do next, including destructive lifecycle work the user asked for (removing a project, deleting a workspace); a statement that the thread is ready ('Le thread est prêt.'); a request to confirm that irreversible work before it runs, which is a genuine question; an announcement of irreversible work with no confirmation request, since the thread confirms the exact paths before removing anything. Fail: the bot asks the user to reply, answer, confirm or send a message so that the bot can start or launch the session, without asking for any specific missing information (for example 'reply here so I can start', 'réponds ici pour que je démarre la session', 'say go'); a claim that this channel session itself handles, follows or takes care of the work ('Je m'en occupe dans ce fil.'); invented details; omitted constraints the user supplied; the bot claiming, in the past tense, that it has already done repository work (created a branch or workspace, committed, pushed, deleted, read or changed code).";
