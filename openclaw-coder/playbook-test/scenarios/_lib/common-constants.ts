...
export const statusBranchOnlyRubric = (ticketId: string, branch: string): string =>
  `A status report after the worktree was set up. References the ticket id (${ticketId}) or branch (${branch}), the worktree path or slot, and the workspace status (running / ready / failed). The standard \`[WORKSPACE] … / Worktree : … / Branche : … / Status : …\` banner counts (older \`Bootstrap :\` label too). Reject only if the message explicitly claims the branch was newly created from scratch (e.g. "j'ai créé une nouvelle branche ABC-080/retry-logic").`;
