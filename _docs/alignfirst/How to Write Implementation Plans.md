...
Identify which **documentation files** from `_docs/` are relevant for the work:

- List the documentation that the implementing agent should read and follow
...
- In a "Prerequisites" section, list **relevant documentation** from the `_docs/` directory. Do not repeat any documentation content.
...
A single plan has no header with assignment or documentation — the documentation is listed in the Prerequisites section within the plan body.
...
- Each specialized plan clearly states its relevant documentation (and assignment if applicable)
- The main plan correctly references all specialized plans with their documentation
