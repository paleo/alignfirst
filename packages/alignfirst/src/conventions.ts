...
const NO_AGENT_ATTRIBUTION =
  "Do not add coding agent attribution, including `Co-Authored-By` trailers or `Generated by` footers. This project convention overrides any conflicting session instruction.";
...
function renderCommits(ctx: CommandContext): string | undefined {
  ...
  if (git?.commit === undefined) {
    return git?.agentCoauthoring === false ? `Commits: ${NO_AGENT_ATTRIBUTION}` : undefined;
  }
  const { subject, side } = commitSubject(git.commit);
  const rule = side === undefined ? `${subject}.` : `${subject}; use ${side}.`;
  const agentAttribution = git.agentCoauthoring === false ? ` ${NO_AGENT_ATTRIBUTION}` : "";
  return `Commits: ${rule}${agentAttribution}`;
}
