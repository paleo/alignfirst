...
describe("conventions command", () => {
  it("renders configured conventions, local work files, and ignored searches", async () => {
  ...
  it("renders the agent attribution rule without a configured commit format", async () => {
    ...
    writeFileSync(
      join(cwd, ".alignfirst.json"),
      JSON.stringify({ schemaVersion: 1, git: { agentCoauthoring: false } }),
    );

    expect((await runMain(["conventions"], { cwd })).stdout).toContain(
      "Do not add coding agent attribution",
    );
  });

  it("renders cached and unresolved default branches and omits absent work files", async () => {
    const cwd = makeProject();
    ...
    expect(unresolved.stdout).not.toContain("Work files:");
