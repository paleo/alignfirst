...
**Model selection.** `--model <id|all>` picks the agent model. The value is a **bare id** (the suffix after the last `/`); it resolves to a full ref by suffix-matching `OPENCLAW_TEST_MODELS`, then the CLI renders that ref into `agents.list[id=main].model` of a run-scoped config the gateway boots on (the canonical `openclaw.json` is never mutated). Omitting `--model` uses `OPENCLAW_DEFAULT_TEST_MODEL`. Model is the **outermost** matrix dimension — `--model all` reboots the gateway at each model boundary and stamps the model into the artifact dir name, `report.json`, the cell record, and the summary; the total-cost line then breaks down per model. `all` needs the API key for every referenced provider.
