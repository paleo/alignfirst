...
describe("setup --profile (e2e)", () => {
  ...
  it(
    ...
    () => {
      ...
      const result = runCli(repo, ["setup", "--profile", "assistant"]);
      const output = result.stdout + result.stderr;

      expect(result.status).toBe(0);
      expect(output).toContain("Applied profile assistant.");
      expect(output).toContain("… ready");
  ...
  it(
    ...
    () => {
      const { repo } = fixture(PROFILE_FIXTURE);
      expect(runCli(repo, ["setup", "--profile", "assistant"]).status).toBe(0);
      const first = profileMarker(repo);
      rmSync(join(repo, "profile-applied.txt"));

      const rerun = runCli(repo, ["setup", "--profile", "assistant"]);
  ...
  it(
    ...
    () => {
      ...
      const result = runCli(linked, ["setup", "--profile", "assistant"]);
      expect(result.status).toBe(1);
  ...
  it(
    ...
    () => {
      const { repo } = fixture(PROFILE_FIXTURE);
      const failed = runCli(repo, ["setup", "--profile", "assistant"], { E2E_PROFILE_FAIL: "1" });
      expect(failed.status).toBe(1);
      expect(failed.stderr).toContain('Profile "assistant": e2e profile boom');
  ...
  it(
    ...
    () => {
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain('Unknown profile "nope". Declared profiles: assistant.');
