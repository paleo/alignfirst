...
// Kept identical to `@paleo/alcoach`'s rule; only the env var names differ.
export function resolveMode(input: ModeInput, env: NodeJS.ProcessEnv): ModeResolution | ModeError {
  const url = input.callbackUrl ?? env.WORKSPACE_CALLBACK_URL;
  ...
  return {
    isBackground: true,
    callback: { url, token: env.WORKSPACE_CALLBACK_TOKEN, sessionKey: input.sessionKey },
  };
}
