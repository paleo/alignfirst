/**
 * Vendored from openclaw `src/plugin-sdk/qa-channel-protocol.ts` at version 2026.5.18.
 * That file is intentionally excluded from openclaw's published exports
 * (`dist/plugin-sdk/qa-channel-protocol.*` is in the package's `files` deny list).
 * Keep in sync with upstream when the openclaw pin is bumped.
 */
