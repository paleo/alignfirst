...
As soon as the run is backgrounded, tell the user — in the user's language — that the coding agent is now working in the background and that you will report back when it finishes (e.g. *"The coding agent is running in the background — I'll let you know as soon as it's done."*). Post it even when the user asked to be notified only at completion: this line is the promise of exactly that, not an interruption — a launch with no ack reads as a session gone silent. Deliver this "started" ack exactly like the completion report below: if you set `--meta` (a Discord thread you created this turn), the ack is a `message` `action: "thread-reply"` carrying that thread's `threadId` — your free-form text would stream to the parent channel. In every other case — always on Slack, and on a thread-bound Discord session — the ack is your plain text; do **not** also post it via `message`, that double-posts. Then go available. Do **not** poll.
...
2. **Report the outcome to the user** — your next action after reading the file, before any other tool call. The report relays the coding agent's account; it doesn't vouch for it, so nothing needs to run before you post it. In the user's language, where the work was requested:

   `Coding run {succeeded | failed} — the agent reports: {one-line summary of the Result block}. {Verifying now.}`

   Keep the `Verifying now` tail whenever your platform's instructions prescribe a verification; it announces the step without blocking the report on it. Two delivery cases:
...
Reporting the run is not calling the work done: the report relays the agent's claim, and the `Verifying now` tail is where you announce checking it — the way your platform's instructions prescribe, as its own step after the report is delivered. Any run you launch for that verification is new work with its own completion wake; the wake you were answering is already discharged.
