...
- For a run launched without `--ticket` or `--no-ticket`, pass `--meta <KEY>` with this session's key. `alcode status --meta <KEY>` then finds its result among concurrent runs in the shared main worktree.
