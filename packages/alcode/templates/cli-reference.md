...
| `--no-ticket` | Work without a ticket: `alcode` reserves the next side ticket `side-N` under `.plans/` and passes it to the agent. `new` only, with a protocol. The reserved id is in the session file's path and `ticket:` frontmatter; pass it as `--ticket side-N` in later runs. |
...
`alcode status` works without a `.plans` directory and does not start a coding session. Its output follows the selected agent's available account limits.
