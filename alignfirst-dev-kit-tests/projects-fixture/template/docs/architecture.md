...
`app.mjs` reads its port from the `PORT` env var or from `local.env` (the per-workspace config `workspace setup` generates), resolves the current branch at startup — so each worktree's server identifies itself when several workspaces run in parallel — and wires two routes:
